{if $__wcf->getUser()->userID && $__wcf->getSession()->getPermission('user.toDo.warning.canSee')}
	...
	{if $waitingToDoCount|isset && $waitingToDoCount > 0 && TODO_WAITING_WARNING}
		<p class="jsOnly error toDoWarning" id="todoWarningWaiting">{$waitingToDoCount} {lang}wcf.toDo.task.waiting.reminder{if $waitingToDoCount == 1}Singular{else}Plural{/if}{/lang}{if $__wcf->getSession()->getPermission('user.toDo.warning.canHide')}<a><span class="icon icon-remove pointer" style="float:right;"></span></a>{/if}</p>
	{/if}
