					...
					<ul>
						{foreach from=$objects item=task}
							<li class="wbbBoardContainer todoContainer wbbDepth2 todoDepth2">
								<div class="{cycle values='wbbBoardNode1,wbbBoardNode2'} {cycle values='todoNode1,todoNode2'} wbbBoard todo box32">
									<div>
										<div class="containerHeadline">
											<h3>{if $__wcf->getSession()->getPermission('user.toDo.toDo.canViewDetail')}<a href="{link controller='ToDo' object=$task}{/link}">{$task->title}</a>{else}{$task->title}{/if}</h3>
