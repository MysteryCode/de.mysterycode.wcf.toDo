				...
				<li class="wbbCategory wbbDepth1 tabularBox tabularBoxTitle todoDepth1">
					...
					<ul>
						{foreach from=$objects item=task}
							<li class="wbbBoardContainer todoContainer wbbDepth2 todoDepth2">
								<div class="{cycle values='todoNode1,todoNode2'} wbbBoard todo box32">
									<div>
										<div class="containerHeadline">
											<h3 class="{if $task->important == 1}importantToDo{/if}">{if $__wcf->getSession()->getPermission('user.toDo.toDo.canViewDetail')}<a href="{link controller='ToDo' object=$task}{/link}">{$task->title}</a>{else}{$task->title}{/if}</h3>
										...
										{if $task->getResponsibles() && $__wcf->getSession()->getPermission('user.toDo.responsible.canView')}
					...
					{foreach from=$objects item=task}
						<tr>
							...
							<td class="columnTitle columnToDoTitle {if $task->important == 1}importantToDo{/if}">
								{if $__wcf->getSession()->getPermission('user.toDo.toDo.canViewDetail')}<a href="{link controller='ToDo' object=$task}{/link}">{$task->title}</a>{else}{$task->title}{/if}</td>
							{if $__wcf->getSession()->getPermission('user.toDo.responsible.canView')}
