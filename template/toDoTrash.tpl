...
<head>
	<title>{lang}wcf.toDo.trash{/lang} - {PAGE_TITLE|language}</title>
	
	{include file='headInclude'}
	
	{capture assign='canonicalURLParameters'}sortField={@$sortField}&sortOrder={@$sortOrder}{/capture}
	
	<link rel="canonical" href="{link controller='ToDoTrash'}{/link}" />
...
<header class="boxHeadline">
	<h1>{lang}wcf.toDo.trash{/lang}</h1>
...
<div class="contentNavigation">
	{pages print=true assign=pagesLinks controller='ToDoTrash' link="pageNo=%d&sortField=$sortField&sortOrder=$sortOrder"}
				...
				<tbody>
					{foreach from=$objects item=task}
						<tr>
							...
							{if $__wcf->getSession()->getPermission('user.toDo.responsible.canView') && $task->getResponsibles()}
