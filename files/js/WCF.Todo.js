		...
		.extend({
			...
			update : function(todoID, data, reloadClipboard) {
				if (!this._todos[todoID]) {
					console.debug("[WCF.Todo.UpdateHandler] Unknown todo id "
							+ todoID);
					return;
				}
			...
			_handleCustomProperty : function(todoID, property, value) {
				this._todos[todoID].trigger('todoUpdateHandlerProperty', [
						todoID, property, value ]);
			},
 ...
 *            elementSelector
 */
WCF.Todo.InlineEditor = WCF.InlineEditor
		.extend({
			/**
			...
			_execute : function(elementID, optionName) {
				...
				case 'delete':
					WCF.System.Confirmation.show(WCF.Language
							.get('wcf.todo.confirmDelete'), $.proxy(function(
							action) {
						if (action == 'confirm') {
							this._updateTodo(elementID, optionName, {
								deleted : 1
				...
				if (optionName == 'delete') {
					new WCF.Action.Proxy({
						...
						success : $.proxy(function(data) {
							this._updateHandler.update($todoID,
			...
			init : function(updateHandler) {
				...
				$('.jsClipboardEditor').each(
						$.proxy(function(index, container) {
							...
							if (WCF.inArray('de.mysterycode.wcf.toDo.toDo',
									$types)) {
...
WCF.Todo.Participate = Class
		.extend({
			...
			init : function(buttonSelector) {
				...
				WCF.DOMNodeInsertedHandler.addCallback('WCF.Todo.Participate',
